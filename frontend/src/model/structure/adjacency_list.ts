...
export const CreateGraph = (vertices: Array<Vertex>): Graph => {
  ...
  for (let vertex in vertices) {
    ...
    adjList.push(
      AdjacencyListItem(vertices[vertex].name, vertices[vertex].distances)
    );
  }
  ...
  const insert = (
    ...
    for (let adjItem in adjList) {
      if (adjList[adjItem].vertex.name === srcVertexName) {
        // Achou onde a aresta deve ser inserida.
   ...
   */
  const getVertex = (vertexName: string): AdjacencyListItem | undefined => {
    for (let adjItem in adjList) {
