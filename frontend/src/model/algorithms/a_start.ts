...
export const a_star = (graph: Graph): AStar => {
  ...
  const h = (srcVertex: string, dstVertex: string): number => {
    ...
    if (typeof vertex !== "undefined") {
      ...
      for (let d in distances) {
        if (distances[d].dstVertex === dstVertex)
  ...
  const preparations = (start_vertex: string, end_vertex: string): Arrays => {
    ...
    for (let v in graph.graph) {
      function_costs.push(Cost(graph.graph[v].vertex.name, INFINITY));
      past_costs.push(Cost(graph.graph[v].vertex.name, INFINITY));
    }

    // Preenchimento do custo estimado e custo passado do vértice inicial.
    let start_function_cost_position = find_vertex_position(
      function_costs,
      start_vertex
    );
    let start_past_cost_position = find_vertex_position(
      past_costs,
      start_vertex
    );
    ...
    while (priority_queue.length != 0) {
      ...
      if (typeof vertex !== "undefined") {
        for (let e in vertex.edges.list) {
          ...
          let try_cost =
            get_vertex_cost(prep.past_costs, next) + neighbor_weigh;
          if (try_cost < get_vertex_cost(prep.past_costs, neighbor_vertex)) {
            ...
            let past_pos = find_vertex_position(
              prep.past_costs,
              neighbor_vertex
            );
            prep.past_costs[past_pos].cost = try_cost;
            // Atribuição do custo passado + estimativa (g(n) + h(n)) para o vizinho.
            let function_pos = find_vertex_position(
              prep.function_costs,
              neighbor_vertex
            );
            prep.function_costs[function_pos].cost =
              try_cost + h(neighbor_vertex, end_vertex);
            ...
            priority_queue.push(
              Cost(neighbor_vertex, prep.function_costs[function_pos].cost)
            );
          }
        }
      } else {
