 ...
 */
const EntryFileContent = (srcVertex: string, dstVertex: string, distance: number): EntryFileContent => {
    return {
        srcVertex,
        dstVertex,
        distance
 ...
 */
const extract_entry_file_content = (content: string): Array<EntryFileContent> => {
...
const extract_h_entry_file_content = (content: string): Array<Vertex> => {
    ...
    let lines = content.split("\n");
    for(let line in lines) { // Processamento da leitura do arquivo.
        let found = false;
        if(lines[line].length != 0) {
            let srcVertex = String(lines[line].split("(")[1].split(",")[0]).replace(/\s/g, '');
 ...
 */
export const process_entry_file = (content: string, h_content: string): Graph => {
    ...
    for(let e in entry) { // Percorre todos os nós carregados.
    ...
    for(let point in points) { 
        ...
        for(let h in entry_h) { // Percorre todas as distâncias euclidianas.
